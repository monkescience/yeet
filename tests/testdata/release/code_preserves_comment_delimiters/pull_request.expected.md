...
## [v1.1.0](http://127.0.0.1:{{regex `\d+`}}/testorg/testrepo/compare/v1.0.0...{{regex `[0-9a-f]{40}`}}) ({{regex `\d{4}-\d{2}-\d{2}`}})

### Features

- add flag ([{{regex `[0-9a-f]{7}`}}](http://127.0.0.1:{{regex `\d+`}}/testorg/testrepo/commit/{{regex `[0-9a-f]{40}`}}))
  > ```text
  > A --> B
  > <!-- example -->
  > ```
