...
```sh
go install github.com/monkescience/yeet/cmd/yeet@v0.9.2 # x-yeet-version
```

Or use the published container image:

```sh
docker run --rm ghcr.io/monkescience/yeet:v0.9.2 --help # x-yeet-version
```
...
VERSION = "0.9.2" # x-yeet-version
MAJOR = 0 # x-yeet-major
MINOR = 9 # x-yeet-minor
PATCH = 2 # x-yeet-patch

# block markers
# x-yeet-start-version
image: ghcr.io/acme/app:0.9.2
appVersion: "0.9.2"
# x-yeet-end
```
...
jobs:
  release:
    ...
    steps:
      ...
      - name: Run yeet
        uses: docker://ghcr.io/monkescience/yeet:v0.9.2 # x-yeet-version
        with:
          args: release
        env:
...
release:
  stage: release
  image:
    name: ghcr.io/monkescience/yeet:v0.9.2 # x-yeet-version
    entrypoint: [""]
  variables:
    GIT_STRATEGY: fetch
    GIT_DEPTH: "0"
...
steps:
  ...
  - script: |
      docker run --rm \
        ...
        ghcr.io/monkescience/yeet:v0.9.2 release # x-yeet-version
