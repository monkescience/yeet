...
```sh
go install github.com/monkescience/yeet/cmd/yeet@v0.6.3-beta.1 # x-yeet-version
```

Or use the published container image:

```sh
docker run --rm ghcr.io/monkescience/yeet:v0.6.3-beta.1 --help # x-yeet-version
```
...
```txt
# inline markers
VERSION = "0.6.3-beta.1" # x-yeet-version
...
image: ghcr.io/acme/app:0.6.3-beta.1
...
jobs:
  release:
    ...
    steps:
      ...
      - name: Run yeet
        uses: docker://ghcr.io/monkescience/yeet:v0.6.3-beta.1 # x-yeet-version
        with:
          args: release
        env:
...
release:
  stage: release
  image:
    name: ghcr.io/monkescience/yeet:v0.6.3-beta.1 # x-yeet-version
    entrypoint: [""]
  variables:
    GIT_STRATEGY: fetch
    GIT_DEPTH: "0"
