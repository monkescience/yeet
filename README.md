...
```sh
go install github.com/monkescience/yeet/cmd/yeet@v0.4.4 # x-yeet-version
```

Or use the published container image:

```sh
docker run --rm ghcr.io/monkescience/yeet:v0.4.4 --help # x-yeet-version
```
...
release:
  stage: release
  image:
    name: ghcr.io/monkescience/yeet:v0.4.4 # x-yeet-version
    entrypoint: [""]
  variables:
    GIT_STRATEGY: fetch
    GIT_DEPTH: "0"
