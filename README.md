...
```sh
go install github.com/monkescience/yeet/cmd/yeet@v0.4.12 # x-yeet-version
```

Or use the published container image:

```sh
docker run --rm ghcr.io/monkescience/yeet:v0.4.12 --help # x-yeet-version
```
...
```txt
# inline markers
VERSION = "0.4.12" # x-yeet-version
MAJOR = 0 # x-yeet-major
MINOR = 4 # x-yeet-minor
PATCH = 12 # x-yeet-patch

# block markers
# x-yeet-start-version
image: ghcr.io/acme/app:0.4.12
appVersion: "0.4.12"
# x-yeet-end
```
...
jobs:
  release:
    ...
    steps:
      ...
      - name: Run yeet
        uses: docker://ghcr.io/monkescience/yeet:v0.4.12 # x-yeet-version
        with:
          args: release
        env:
...
release:
  stage: release
  image:
    name: ghcr.io/monkescience/yeet:v0.4.12 # x-yeet-version
    entrypoint: [""]
  variables:
    GIT_STRATEGY: fetch
    GIT_DEPTH: "0"
