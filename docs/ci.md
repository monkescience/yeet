...
jobs:
  release:
    ...
    steps:
      ...
      - name: Run yeet
        uses: docker://ghcr.io/monkescience/yeet:v0.13.9-beta.1 # x-yeet-version
        with:
          args: release
        env:
...
release:
  stage: release
  image:
    name: ghcr.io/monkescience/yeet:v0.13.9-beta.1 # x-yeet-version
    entrypoint: [""]
  variables:
    GIT_STRATEGY: fetch
    GIT_DEPTH: "0"
...
steps:
  ...
  - script: |
      docker run --rm \
        ...
        ghcr.io/monkescience/yeet:v0.13.9-beta.1 release # x-yeet-version
